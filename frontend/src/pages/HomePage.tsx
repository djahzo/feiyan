...
/** 默认英雄区副文案：三角洲战术射击向 + 商务可用语境（接口无签名时兜底） */
const DEFAULT_SIGN_FALLBACK =
...
function HeaderActions() {
  return (
    <a
      href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(`【商务合作咨询】${SITE_NAME}`)}`}
      className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-[#333] transition hover:border-brand-link hover:text-brand-link"
    >
      商务询价
    </a>
  );
...
const HomePage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [u, l, v] = await Promise.all([
        bilibiliApi.getUserInfo().catch((e) => {
          console.warn(e);
          return null;
        }),
  ...
  const spaceUrl = useMemo(
    () => (userInfo?.mid ? `https://space.bilibili.com/${userInfo.mid}` : 'https://www.bilibili.com'),
    [userInfo?.mid]
  );

  const heroSubtitle = (userInfo?.sign && userInfo.sign.trim()) || DEFAULT_SIGN_FALLBACK;
  const faceUrl = userInfo?.face || '/hero-portrait-placeholder.svg';
  ...
  return (
    <div className="min-h-screen bg-white text-[#333]">
      <header className="sticky top-0 z-50 border-b border-gray-100/80 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3.5 md:px-8">
          <a href="#intro" className="font-bold tracking-tight">
            {SITE_NAME}
            <span className="ml-2 hidden font-normal text-[#888] sm:inline">· 商务主站</span>
          </a>
          <nav className="hidden flex-1 justify-center md:flex">
            <ul className="flex gap-6 text-sm text-[#444] lg:gap-8">
              {navLinks.map((l) => (
                <li key={l.href}>
                  <a href={l.href} className="hover:text-brand-link">
                    {l.label}
                  </a>
                </li>
              ))}
      ...
      {/* 直播中条：游戏 UP 商务站常见的即时入口 */}
      {live && live.live_status === 1 && live.url && (
        <a
          href={live.url}
          target="_blank"
          rel="noopener noreferrer"
          className="block bg-gradient-to-r from-[#1a1f2e] to-brand-dark px-4 py-3 text-center text-sm text-white transition hover:opacity-95"
        >
      ...
      <section
        id="intro"
        className="relative scroll-mt-20 overflow-hidden bg-[radial-gradient(ellipse_at_top_right,_#1a2838_0%,_#0A0E14_55%,_#06080c_100%)]"
      >
        <div className="pointer-events-none absolute -right-24 top-16 h-64 w-64 rounded-full bg-brand-gold/10 blur-3xl" />
        <div className="relative mx-auto flex max-w-5xl flex-col gap-12 px-4 pb-20 pt-16 md:flex-row md:items-stretch md:px-8 md:pb-24 md:pt-20">
          <div className="hidden w-1 shrink-0 rounded-full bg-gradient-to-b from-brand-gold via-brand-gold/40 to-transparent md:block" />
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium tracking-wide text-brand-gold/90">
              B站 UP 主 · {loading ? '…' : displayNick}
            </p>
            <h1 className="mt-4 text-[1.65rem] font-bold leading-snug text-white md:text-4xl md:leading-tight">
              战术射击内容，
              <span className="text-brand-gold">做给玩家与品牌</span>
              同频对话
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              <a
                href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(`【商务合作】${SITE_NAME}`)}`}
                className="inline-flex rounded-lg bg-brand-gold px-6 py-3 text-sm font-semibold text-black shadow-lg/25 transition hover:brightness-110"
              >
                发商务邮件
              </a>
              <a
              ...
              <a href="#biz" className="text-sm font-medium text-white/60 hover:text-white/90">
                合作流程 →
              </a>
            </div>
          </div>
          <div className="flex shrink-0 items-end justify-center md:w-[280px] lg:w-[320px]">
            <div className="relative w-full max-w-xs">
              <div className="absolute -inset-3 rounded-2xl bg-gradient-to-tr from-brand-gold/20 to-transparent" />
              <img
                src={faceUrl}
                alt=""
                className="relative aspect-square w-full rounded-xl object-cover object-center ring-2 ring-white/10"
                loading="eager"
      ...
      <div className="border-y border-gray-100 bg-gray-50/80">
        <div className="mx-auto flex max-w-5xl flex-wrap justify-center gap-8 px-4 py-6 text-center text-sm text-[#555] md:justify-between md:px-8 md:text-left">
          <div>
            <p className="text-xs uppercase tracking-wider text-[#888]">频道昵称</p>
            <p className="mt-1 font-semibold text-[#222]">{loading ? '加载中…' : displayNick}</p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-wider text-[#888]">公开稿件</p>
            <p className="mt-1 font-semibold text-[#222]">
              {loading ? '…' : `${videoTotal || videos.length} 支`}
            </p>
          </div>
          <div>
            <p className="text-xs uppercase tracking-wider text-[#888]">商务联络</p>
            <p className="mt-1 font-semibold text-[#222]">邮件 1–2 个工作日内回复</p>
          </div>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-12 max-w-2xl">
            ...
            <p className="mt-3 text-[#666]">
              面向硬件外设、游戏发行、饮料零食、电竞椅等品牌侧：可按 campaign
              做单场植入，也可按版本节奏签季度框架。以下为常见协作路径，实际以合同与排期为准。
            </p>
          </div>
          <ol className="space-y-10 border-l-2 border-gray-200 pl-8 md:pl-10">
            {bizBlocks.map((b) => (
              <li key={b.k} className="relative">
                <span className="absolute -left-[2.125rem] top-0 flex h-8 w-8 items-center justify-center rounded-full bg-brand-dark text-xs font-bold text-brand-gold md:-left-[2.5rem]">
                  {b.k}
                </span>
      ...
      <section id="trust" className="scroll-mt-20 bg-brand-dark px-4 py-20 md:px-8">
        <div className="mx-auto max-w-5xl">
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-white md:text-3xl">合作说明</h2>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-white/65">
              三角洲玩家圈子认「真实对局与版本理解」。主站用于让品牌方在下单前确认内容风格与合作边界；不代替具体报价与合同条款。
            </p>
          </div>
          <div className="grid gap-8 md:grid-cols-2">
            {trustPoints.map((t) => (
              ...
              >
                <h3 className="font-semibold text-brand-gold">{t.title}</h3>
      ...
      <section id="videos" className="scroll-mt-20 px-4 py-20 md:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mx-auto mt-3 max-w-xl text-center text-sm text-[#666]">
          ...
          {loading ? (
            ...
            <div className="mt-14 flex flex-col items-center rounded-2xl border border-dashed border-gray-200 bg-[linear-gradient(180deg,#fafafa_0%,#f3f4f6_100%)] px-6 py-14 text-center md:py-16">
              ...
              <p className="mt-3 max-w-md text-sm leading-relaxed text-[#666]">
                稿件暂时没在阵地上——可能在坝里、在航天刮地皮，也可能接口在排队。刷新试试，或直达 B 站空间翻仓库。
              </p>
              <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={() => loadData()}
                  className="rounded-lg bg-brand-dark px-5 py-2.5 text-sm font-medium text-white transition hover:bg-brand-dark/90"
                >
                  刷新稿件
                </button>
                {userInfo?.mid && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-medium text-[#333] transition hover:bg-gray-50"
                  >
                    去 B 站空间
                  </a>
                )}
              ...
              {videos.map((v) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h3 className="text-base font-bold leading-snug text-[#222] group-hover:text-brand-link">
                      <a
                        href={`https://www.bilibili.com/video/${v.bvid}`}
                        target="_blank"
                        rel="noopener noreferrer"
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 text-xs text-[#999]">
                      <span>{formatCreated(v.created)}</span>
                      <span>{formatPlay(v.play)} 播放</span>
                      {v.video_review > 0 && <span>{formatPlay(v.video_review)} 评论</span>}
                    </div>
                  </div>
                </article>
              ))}
      ...
      <footer id="contact" className="scroll-mt-20 border-t border-gray-200 bg-gray-50">
        <div className="mx-auto max-w-5xl px-4 py-14 md:px-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-[#666]">
                《三角洲行动》等内容向品牌合作 · 询价请邮件并注明品牌与大致档期
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <a
                href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(`【商务合作】${SITE_NAME}`)}`}
                className="inline-flex w-fit items-center rounded-lg bg-brand-link px-5 py-2.5 text-sm font-medium text-white hover:opacity-90"
              >
                {CONTACT_EMAIL}
              </a>
              {userInfo?.mid && (
          ...
          <p className="mt-10 text-center text-xs text-[#999]">
            © {new Date().getFullYear()} {SITE_NAME} · 本站为展示用途，合作以书面确认与平台规则为准
          </p>
        </div>
      </footer>
